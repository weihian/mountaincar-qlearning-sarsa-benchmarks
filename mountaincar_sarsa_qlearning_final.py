...
NUM_ACTIONS = 3  # (accelerate left, don't accelerate, accelerate right)
...
def state_to_bucket(state):
    ...
    for i in range(len(state)):
        if state[i] <= STATE_BOUNDS[i][0]:
            ...
            #  visualize, i.e. num_buckets x percentage in width.
...
def train():
    ...
    for episode in range(NUM_TRAIN_EPISODES):
        ...
        for t in range(MAX_TRAIN_T):
            ...
            if selection:
            # if episode <= NUM_TRAIN_EPISODES * 0.8:
            ...
            # if episode <= NUM_TRAIN_EPISODES * 0.8:
                ...
                q_table[state_0 + (action_0,)] = q_table[state_0 + (action_0,)] + learning_rate * (
                        reward + discount_factor * q_table[state_1 + (action_1,)] - q_table[state_0 + (action_0,)])
